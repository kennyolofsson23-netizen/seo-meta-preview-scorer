  ...
  try {
    ...
    if (textareas.length > 0) {
      await textareas[0].fill(
        "This is a test meta description for SEO scoring.",
      );
    ...
    console.log(
      `  ${charCounters > 0 ? "✓" : "✗"} Character counters: ${charCounters} found`,
    );
    ...
    const hasGooglePreview =
      googleDesktopContent.includes("Google") ||
      googleDesktopContent.includes("google");
    ...
    console.log(
      `  ${scoreElements > 0 ? "✓" : "✗"} Score elements: ${scoreElements}`,
    );
    if (scoreElements > 0) {
    ...
    for (const btn of buttons) {
      ...
      if (
        (title && (title.includes("theme") || title.includes("dark"))) ||
    ...
    try {
      ...
      const hasEmbedCode =
        (await page.locator("text=/iframe|embed|code|Get Embed/i").count()) > 0;
    ...
    const historyElements = await page
      .locator("text=/History|recent/i")
      .count();
    console.log(
      `  ${historyElements > 0 ? "✓" : "✗"} History panel: ${historyElements}`,
    );
