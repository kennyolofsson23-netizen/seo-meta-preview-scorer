  ...
  try {
    ...
    if (hasInputs && hasTextarea) {
      ...
      if (counters && counters.length >= 2) {
        console.log(
          `  ✓ Character counting: ${counters.length} counters found`,
        );
      }
    } else {
    ...
    if (hasDownload || content.includes("Download")) {
      ...
      results.escalations.push(
        "[ESCALATION] F008 - Screenshot export not found",
      );
    }
    ...
    if (await themeBtn.isVisible()) {
      ...
      results.escalations.push(
        "[ESCALATION] F010 - Dark mode toggle not found",
      );
    }
    ...
    if (
      ...
      results.warnings.push(
        "[WARN] F011 - Affiliate recommendations not found",
      );
    }
    ...
    // F015: URL Auto-Fetch
    console.log("\n[F015] URL Auto-Fetch");
    const importBtn = buttons.some(
      (b) => b.includes("Import") || b.includes("Fetch"),
    );
    if (importBtn) {
      ...
      results.warnings.push(
        "[WARN] F015 - URL fetch button not clearly visible",
      );
    }
    ...
    const avgScore =
      Object.values(overallScore).reduce((a, b) => a + b) /
    ...
    const verdict =
      results.blockers.length === 0 && results.escalations.length === 0
        ? "PASS"
        : "FAIL";
    console.log(`\n🎯 VERDICT: ${verdict}`);
  }
