...
async function test() {
  ...
  if (inputs.length >= 2) {
    ...
    await inputs[1].fill('Learn how to optimize your blog posts for search engines and increase traffic');
  ...
  const scoreElements = await page.locator('text=/[Ss]core|Overall|%|\/\d+/').all();
  ...
  for (const btn of buttons) {
    ...
    if (text && (text.includes('Dark') || text.includes('Light') || text.includes('🌙') || text.includes('☀'))) {
      themeFound = true;
  ...
  console.log('✓ Navigation links:', linkTexts.filter(t => t && t.trim().length > 0 && t.trim().length < 50).slice(0, 8).join(' | '));
