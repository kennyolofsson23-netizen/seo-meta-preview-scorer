...
async function test() {
  ...
  try {
    ...
    const themeBtn = await page.locator('button').filter({ hasText: /🌙|☀|dark|light|theme/i }).first();
    if (themeBtn) {
  ...
  try {
    ...
    if (inputs.length >= 2) {
      ...
      if (counterText.includes('characters') || counterText.includes('chars') || /\d+\/\d+/.test(counterText)) {
        logResult('F001-CharCounter', 'OK', 'Character counter updates in real-time');
      }

      // Test description input
      if (inputs.length >= 2) {
        await inputs[1].fill('Learn the best practices for optimizing your blog post titles and descriptions for search engines');
        logResult('F001-DescInput', 'OK', 'Description field accepts input');
      }
    } else {
      logResult('F001', 'BLOCKER', `Only found ${inputs.length} inputs, expected 2+`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (tabs.length > 0) {
      logResult('F007', 'OK', `Found ${tabs.length} preview tabs: ${tabs.join(', ')}`);

      // Check for each expected preview type
      const tabsStr = tabs.join(' ').toLowerCase();

      if (tabsStr.includes('google') && (tabsStr.includes('desktop') || /google.*[123]/.test(tabsStr))) {
      ...
      if (tabsStr.includes('social') || tabsStr.includes('og') || tabsStr.includes('card')) {
  ...
  try {
    ...
    if (bodyText.includes('Score') || bodyText.includes('score')) {
      ...
      if (bodyText.includes('%') || /\d+\/\d+/.test(bodyText)) {
        logResult('F004-ScoreFormat', 'OK', 'Scores display with numeric format');
      }

      if (bodyText.includes('Title') || bodyText.includes('Description')) {
        logResult('F004-Components', 'OK', 'Individual score components visible');
      }
    } else {
  ...
  console.log('\n--- F010: Dark Mode ---\n');
  try {
    const initialClass = await page.locator('html').evaluate(el => el.className);
    const themeBtn = await page.locator('button').filter({ hasText: /🌙|☀|dark|light|theme/i }).first();
    ...
    if (themeBtn) {
      ...
      const newClass = await page.locator('html').evaluate(el => el.className);
  ...
  try {
    const downloadBtn = await page.locator('button').filter({ hasText: /download|screenshot|export/i }).first();

    if (downloadBtn) {
      logResult('F008', 'OK', 'Screenshot/Download button found');
    } else {
      logResult('F008', 'WARN', 'Download button not found in obvious location');
    }
  } catch (e) {
  ...
  try {
    ...
    const embedLink = await page.locator('a, button').filter({ hasText: /embed|widget/i }).first();
    ...
    if (embedLink) {
      ...
      if (href && href.includes('/embed')) {
        ...
        if (embedContent.includes('embed') || embedContent.includes('code') || embedContent.includes('iframe')) {
  ...
  try {
    ...
    const historyLink = await page.locator('a, button').filter({ hasText: /history|recent/i }).first();
    ...
    if (historyLink) {
      ...
      if (historyContent.includes('history') || historyContent.includes('check')) {
  ...
  console.log('\n--- F014: Bulk CSV Check ---\n');
  try {
    const bulkLink = await page.locator('a, button').filter({ hasText: /bulk|csv/i }).first();
  ...
  console.log('\n--- F015: URL Auto-Fetch ---\n');
  try {
    const fetchBtn = await page.locator('button').filter({ hasText: /fetch|import/i }).first();
  ...
  try {
    const response = await page.goto('http://localhost:3011/api/og?title=Test&score=85');
    if (response && response.status() === 200) {
      const contentType = response.headers()['content-type'] || '';
      if (contentType.includes('image')) {
        logResult('F012', 'OK', 'OG image API endpoint works');
      } else {
        logResult('F012', 'WARN', 'OG endpoint responds but may not return image');
      }
    }
  } catch (e) {
