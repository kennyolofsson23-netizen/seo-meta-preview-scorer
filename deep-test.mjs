...
async function runDeepTest() {
  ...
  try {
    ...
    const screenshotButtons = await page.locator('button:has-text("Screenshot"), button:has-text("Download"), button:has-text("Export")').count();
    console.log(`Screenshot-related buttons: ${screenshotButtons}`);

    // Check embed page
    console.log('\n=== Testing Embed Page ===');
    const embedResponse = await page.goto(`${BASE_URL}/embed`, { waitUntil: 'networkidle' }).catch(e => null);
    if (embedResponse && embedResponse.status() < 400) {
      ...
      const embedCodeArea = await page.locator('code, textarea, pre, [class*="code"]').count();
    ...
    await descTextarea.fill('Discover the top 10 coffee shops in Brooklyn with reviews, ratings, and insider tips for the best espresso and pastries.');
