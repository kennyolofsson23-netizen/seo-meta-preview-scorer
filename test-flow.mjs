...
async function testApp() {
  ...
  await descField.fill('Learn the top 10 Python tips that will make you a better programmer. Includes code examples and best practices.');
  ...
  const scoreElements = await page.locator('text=/[0-9]{1,3}(\.[0-9])?(\s*\/\s*100)?/').count();
  console.log(`✓ Found ${scoreElements} score-related elements`);

  // Test 8: Theme toggle
  console.log('\nTEST 8: Test theme toggle');
  const themeButtons = await page.locator('button[aria-label*="theme" i]').count();
  if (themeButtons > 0) {
  ...
  const keywordInputs = page.locator('input[placeholder*="keyword" i], input[placeholder*="Keyword" i]');
  ...
  const exportButtons = await page.locator('button:has-text("Download"), button:has-text("Export"), button:has-text("Screenshot")').count();
  ...
  const embedLinks = await page.locator('a[href*="embed"], a[href*="widget"]').count();
