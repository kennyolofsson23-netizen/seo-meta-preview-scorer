...
(async () => {
  ...
  try {
    ...
    const titleInput = await page.$('input[placeholder*="e.g."]') || await page.$('input[type="text"]');
    if (titleInput) {
    ...
    if (descInput) {
      await page.fill('textarea', 'Learn the latest SEO best practices to rank higher in Google search results.');
    ...
    console.log(`✓ Found ${tabs.length} preview tab(s): ${tabs.map(t => t.textContent).join(', ')}`);
    ...
    const darkModeBtn = await page.$('button[title*="dark"]') || await page.$('button[title*="theme"]') || await page.$('[aria-label*="theme"]');
    if (darkModeBtn) {
    ...
    const screenshotBtn = await page.$('button:has-text("Screenshot")') || await page.$('text=/screenshot|Download/i');
    if (screenshotBtn) {
    ...
    await page.goto('http://localhost:3009/embed', { waitUntil: 'networkidle', timeout: 5000 }).catch(() => {
      console.log('⚠ /embed page not found');
    });

    // Test 11: Check for widget page
    console.log('\n[TEST 11] Checking widget page...');
    await page.goto('http://localhost:3009/widget', { waitUntil: 'networkidle', timeout: 5000 }).catch(() => {
