...
async function detailedFeatureTest() {
  ...
  try {
    ...
    await textarea.fill(
      "Discover the top-rated coffee shops in Brooklyn with unique atmospheres and delicious espresso drinks.",
    );
    ...
    if (await downloadBtn.isEnabled()) {
      ...
      try {
        ...
        console.log(
          `  ✓ Screenshot download initiated: ${download.suggestedFilename()}`,
        );
      } catch (e) {
        console.log(
          `  ⚠️  Download dialog did not appear (this may be expected)`,
        );
      }
    } else {
    ...
    if (await historyTab.isVisible()) {
      ...
      const historyContent = await page
        .locator("text=/coffee|brooklyn/i")
        .count();
      if (historyContent > 0) {
    ...
    const affiliateLinks = await page
      .locator(
        'a[rel*="sponsored"], a:has-text("Ahrefs"), a:has-text("Semrush"), a:has-text("SurferSEO")',
      )
      .count();
    console.log(
      `  ${affiliateLinks > 0 ? "✓" : "⚠️"} Affiliate CTAs: ${affiliateLinks}`,
    );
    ...
    const codeBlock = await page
      .locator("code, textarea[readonly], textarea[disabled]")
      .first();
    ...
    await descInput.fill(
      "Discover the best coffee shops in Brooklyn with ratings, reviews, and insider tips for finding the perfect espresso and pastries.",
    );
    ...
    const hasGreenScore =
      pageText.includes("Green") || pageText.includes("Optimal");
    const hasYellowScore = pageText.includes("Yellow");
    const hasRedScore = pageText.includes("Red");
    const hasFeedback =
      pageText.includes("character") || pageText.includes("length");
    ...
    console.log("\nTEST 6: Mobile Truncation Warning");
    const longTitle =
      "This is an extremely long title that definitely exceeds the mobile character limit and should trigger a warning";
    await titleInput.fill(longTitle);
    await page.waitForTimeout(300);

    const warningText = await page.evaluate(() => document.body.innerText);
    const hasMobileWarning =
      warningText.includes("mobile") ||
      warningText.includes("Mobile") ||
      warningText.includes("truncate");
    console.log(
      `  ${hasMobileWarning ? "✓" : "⚠️"} Mobile truncation warning appears for long title`,
    );
    ...
    const hasKeywordCheck =
      keywordCheckText.includes("keyword") ||
      keywordCheckText.includes("Keyword");
    ...
    for (let i = 0; i < Math.min(4, tabs.length); i++) {
      ...
      if (
        (tabText && tabText.includes("Google")) ||
        tabText.includes("Bing") ||
        tabText.includes("Social")
      ) {
        await tabs[i].click();
    ...
    if (await themeBtn.isVisible()) {
      ...
      if (bodyBefore !== bodyAfter) {
        console.log(
          `  ✓ Theme switched (before: ${bodyBefore}, after: ${bodyAfter})`,
        );
      } else {
        console.log(
          `  ⚠️  Theme toggle clicked but colors may not have changed`,
        );
      }
    }
