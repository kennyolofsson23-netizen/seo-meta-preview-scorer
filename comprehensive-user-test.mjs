...
async function comprehensiveTest() {
  ...
  try {
    ...
    const testDesc = 'Discover the best SEO tools to improve your ranking, analyze competitors, and optimize your content. Reviews, pricing, and features comparison.';
    ...
    const googleTab = await page.locator('[role="tab"]:has-text("Google")').first();
    if (await googleTab.isVisible()) {
      await googleTab.click();
      await page.waitForTimeout(300);

      const googlePreview = await page.locator('text=/example.com|seo tools/i').count();
      if (googlePreview > 0) {
        results.passes.push('F002: Google Desktop preview renders');
        console.log('  ✓ Google Desktop preview displays');
      } else {
        results.blockers.push('[BLOCKER] Google Desktop preview not showing content');
      }
    }

    // ===== CORE FLOW 3: Google Mobile Preview =====
    console.log('\nFLOW 3: Google Mobile SERP Preview');
    const mobileTab = await page.locator('[role="tab"]:has-text("Mobile")').first();
    if (await mobileTab.isVisible()) {
      await mobileTab.click();
      await page.waitForTimeout(300);

      const mobileContent = await page.locator('text=/mobile|Mobile/i, text=/example.com/').count();
      if (mobileContent > 0) {
    ...
    if (await bingTab.isVisible()) {
      ...
      const bingContent = await page.locator('text=/bing|Bing/i, text=/example.com/').count();
      if (bingContent > 0) {
    ...
    const socialTab = await page.locator('[role="tab"]:has-text("Social")').first();
    if (await socialTab.isVisible()) {
      await socialTab.click();
      await page.waitForTimeout(300);

      const socialContent = await page.locator('text=/social|facebook|linkedin|card/i').count();
      if (socialContent >= 0) {
    ...
    const scoreTab = await page.locator('[role="tab"]:has-text("Preview & Score")').first();
    if (await scoreTab.isVisible()) {
      await scoreTab.click();
      await page.waitForTimeout(300);

      const scoreText = await page.evaluate(() => document.body.innerText);
      const hasScore = scoreText.includes('score') || scoreText.includes('Score');
      const hasColorRating = scoreText.includes('Green') || scoreText.includes('Yellow') || scoreText.includes('Red');
    ...
    // ===== FEATURE: Dark Mode =====
    console.log('\nFEATURE: Dark Mode');
    const themeBtn = await page.locator('button[aria-label*="dark" i], button[aria-label*="theme" i]').first();
    ...
    console.log('\nFEATURE: Screenshot Export');
    const downloadBtn = await page.locator('button:has-text("Download"), button[aria-label*="Download"]').first();
    if (await downloadBtn.isVisible()) {
      results.passes.push('F008: Download button visible');
      console.log('  ✓ Download button present');
    } else {
      results.escalations.push('[ESCALATION] Screenshot export button not visible');
    }
    ...
    const fetchBtn = await page.locator('button:has-text("Fetch"), button[aria-label*="Import"]').first();
    ...
    if (urlInputs.length > 0) {
      ...
      const validationMsg = await page.locator('text=/invalid|error|valid/i').count();
      if (validationMsg > 0) {
    ...
    const longTitle = 'This is a very long title that should be truncated in the Google search result preview because it exceeds the maximum character limit';
