...
describe('GoogleMobilePreview', () => {
  ...
  it('renders without phone frame', () => {
    const { container } = render(<GoogleMobilePreview {...defaultProps} showPhoneFrame={false} />)
  ...
  it('shows mobile truncation warning when title > 50 chars', () => {
    const longTitle = 'A'.repeat(60)
    render(<GoogleMobilePreview title={longTitle} description="Short" url="https://example.com" showPhoneFrame={true} />)
    expect(screen.getByRole('alert')).toBeTruthy()
    expect(screen.getByText(/Mobile truncation detected/)).toBeTruthy()
  })

  it('does not show warning when content within limits', () => {
    render(<GoogleMobilePreview title="Short title" description="Short desc" url="https://example.com" showPhoneFrame={true} />)
  ...
  it('has correct aria label', () => {
    ...
    const el = container.querySelector('[aria-label="Google mobile search result preview"]')
