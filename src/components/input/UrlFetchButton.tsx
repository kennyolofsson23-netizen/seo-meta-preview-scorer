...
export function UrlFetchButton({ url, onFetch, disabled = false }: UrlFetchButtonProps) {
  ...
  async function handleFetch() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error ?? 'Failed to fetch meta data. Please enter it manually.')
        return
      }
  ...
  return (
    <div className="flex flex-col gap-1">
      <Button
        ...
        onClick={handleFetch}
        disabled={isDisabled}
        aria-label={loading ? 'Fetching meta data...' : 'Fetch meta data from URL'}
      >
        {loading ? (
