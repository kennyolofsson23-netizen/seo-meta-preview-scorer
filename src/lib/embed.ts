 ...
 */
export function generateEmbedCode(options: Partial<WidgetOptions> = {}): string {
  ...
  if (options.defaultDescription) params.set('description', options.defaultDescription)
 ...
 */
export function parseWidgetOptions(searchParams: URLSearchParams): Partial<WidgetOptions> {
  const options: Partial<WidgetOptions> = {}

  if (searchParams.get('showScores') === 'false') options.showScores = false
  if (searchParams.get('showPreviews') === 'false') options.showPreviews = false
